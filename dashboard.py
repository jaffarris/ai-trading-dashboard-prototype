...
from indicators import (build_alerts, build_price_movement_analysis, build_trade_analysis,
...
h1,h2,h3,p{margin:0}.terminal-head{height:35px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid var(--line);margin-bottom:2px}.brand{font-size:19px;font-weight:850;letter-spacing:.4px}.brand b{color:var(--cyan)}.status{font-size:11px;color:var(--muted)}.live{color:var(--green);font-weight:800}
...
.scanner-table,.pattern-table,.movement-table,.alert-table{width:100%;border-collapse:collapse;table-layout:fixed}th{font-size:8px;color:#738197;text-align:left;padding:4px 3px;border-bottom:1px solid var(--line);text-transform:uppercase}td{font-size:10.5px;padding:5.5px 3px;border-bottom:1px solid #172231;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.ticker{font-weight:850;color:var(--cyan)}.score{font-weight:850}.scanner-table a{display:block;color:inherit;text-decoration:none;margin:-5.5px -3px;padding:5.5px 3px}.scanner-table tr{cursor:pointer;transition:background .12s ease}.scanner-table tbody tr:hover{background:rgba(53,194,255,.11)}.pattern-table td{padding:3.2px 3px;font-size:9.5px}.pattern-table .hit{background:#12231f}.pattern-icon{font-size:13px;text-align:center}.bias-bullish{color:var(--green)}.bias-bearish{color:var(--red)}.bias-neutral,.bias-directional{color:var(--amber)}
.scanner-table tbody tr.selected{background:rgba(53,194,255,.16);box-shadow:inset 2px 0 var(--cyan)}
.pattern-wrap{position:relative;z-index:50}.pattern-wrap,.pattern-wrap .panel-body,.pattern-table{overflow:visible!important}.pattern-name-cell{position:relative;overflow:visible!important;cursor:help}.pattern-table tr.confirmed{background:#e3f1ff!important;box-shadow:inset 3px 0 #248cff}.pattern-table tr.confirmed td{border-bottom-color:#b9d8f7}.status-confirmed{color:#0877d1;font-weight:900}.status-watch{color:#b87500;font-weight:800}.status-complete{color:#16865f;font-weight:800}.pattern-time{color:#607086;font-variant-numeric:tabular-nums}.pattern-tip{display:none;position:absolute;z-index:100;right:calc(100% + 12px);top:-45px;width:282px;box-sizing:border-box;padding:10px;background:#fff;border:1px solid #bfcbd9;border-radius:8px;box-shadow:0 8px 24px rgba(19,32,51,.22);white-space:normal;color:#26364b;text-align:left}.pattern-table tr:hover .pattern-tip{display:block}.pattern-tip.tip-lift{top:auto;bottom:-32px}.tip-head{display:flex;align-items:center;justify-content:space-between;font-size:11px;margin-bottom:3px}.tip-head span{font-size:8px;font-weight:900;letter-spacing:.5px;padding:2px 5px;border-radius:3px}.snapshot-label{font-size:7px;font-weight:900;letter-spacing:.7px;color:#708096;margin-top:2px}.bull{color:#087d57;background:#ddf6ec}.bear{color:#c72f45;background:#ffe5e9}.neutral-tip{color:#9a6500;background:#fff1cf}.formation-svg{display:block;width:100%;height:92px;background:#f8fafc;border:1px solid #e1e7ef;border-radius:5px;margin:4px 0}.tip-meaning{font-size:10px;line-height:1.35;margin:6px 1px}.tip-confirm{font-size:9px;line-height:1.35;color:#5f6f84;background:#f1f5f9;border-radius:4px;padding:6px}.tip-confirm b{color:#34465e}
...
def show_trade_coach(ticker: str, interval: str) -> None:
    ...
    st.markdown(
        f'<div class="coach-summary"><b>{escape(analysis["option_bias"])}</b> · '
...
def render_live_dashboard(ticker, interval, reference_choice, custom_threshold, chart_focus):
    ...
    if ticker!="SPY":
        try:
            ...
            if not spy.empty: spy_change=percentage_move(float(spy["Close"].iloc[-1]),float(spy["Open"].iloc[0]))
        except Exception: pass
    analysis=build_trade_analysis(data,spy_change); movement=build_price_movement_analysis(data,support,resistance)
    patterns=detect_candlestick_statuses(data); alerts=build_alerts(data,analysis,movement,custom_threshold)
    ...
    direction="BULLISH" if analysis["score"]>=60 else "BEARISH" if analysis["score"]<=40 else "NEUTRAL"
    tiles=[metric("Current Price",f"${latest:,.2f}",ticker,"cyan"),metric("Move %",f"{move_pct:+.2f}%",reference_choice,tone(move_pct)),metric("Pullback %",f"{pullback_pct:+.2f}%","20-bar high",tone(pullback_pct)),metric("Move Strength",f"{strength_pct}%",move_strength(abs(session_move)),"cyan"),metric("RVOL-TOD",f"{rvol:.2f}×" if rvol else "N/A",rvol_sub,"up" if 2<=rvol<=4 else "down" if rvol>4 else "neutral"),metric("AI Score",f"{analysis['score']}/100",analysis["signal"],"up" if analysis["score"]>=60 else "down" if analysis["score"]<=40 else "neutral"),metric("Market Direction",direction,"trend model","up" if direction=="BULLISH" else "down" if direction=="BEARISH" else "neutral"),metric("Probability",f"{analysis['probability']}%","model confidence","cyan"),metric("Candlestick",analysis["pattern"],analysis["pattern_bias"],"up" if "Bullish" in analysis["pattern_bias"] else "down" if "Bearish" in analysis["pattern_bias"] else "neutral")]
    st.markdown('<div class="metric-grid">'+''.join(tiles)+'</div>',unsafe_allow_html=True)

    scanner_df=scan_watchlist(); left,center,right=st.columns([1.18,2.55,1.47],gap="small")
    with left:
        rows=""
        for _,r in scanner_df.iterrows():
            ...
            symbol=escape(str(r["Ticker"])); href=f'?symbol={symbol}'; signal=escape(str(r["Signal"]).replace(" WATCH","")); selected="selected" if symbol==ticker else ""
            rows+=f'<tr class="{selected}" title="Open {symbol}"><td class="ticker"><a href="{href}" target="_self">{symbol}</a></td><td><a href="{href}" target="_self">{price}</a></td><td class="{mv_class}"><a href="{href}" target="_self">{move}</a></td><td class="score"><a href="{href}" target="_self">{int(r["Score"])}</a></td><td><a href="{href}" target="_self">{signal}</a></td></tr>'
        st.markdown(f'<div class="panel scanner-wrap"><div class="panel-title"><span>Live Scanner</span><span>{len(scanner_df)} symbols</span></div><div class="panel-body"><table class="scanner-table"><thead><tr><th>Symbol</th><th>Price</th><th>Move</th><th>AI</th><th>Signal</th></tr></thead><tbody>{rows}</tbody></table></div></div>',unsafe_allow_html=True)
    with center:
    ...
    with right:
        ...
        for pattern_index,p in enumerate(patterns):
            ...
            hover_guide=guide_html(p["name"],p["bias"],p["confirmation"],lift=pattern_index>=8,actual_svg=actual_snapshot,snapshot_label=snapshot_label)
    ...
    with b1:
        ...
        for label,value in movement.items():
            probability=label.startswith("Expected"); color="neutral" if probability else tone(value); display=f"{value:.0f}%" if probability else f"{value:+.2f}%"; rows+=f'<tr><td>{escape(label)}</td><td class="{color}">{display}</td></tr>'
    ...
    with b2:
        rows=""
        for a in alerts[:7]: rows+=f'<tr><td class="pri-{a["priority"].lower()}"><b>{a["priority"]}</b></td><td>{escape(a["name"])}</td><td>{a["confidence"]}%</td><td>{a["continuation"]}%</td><td>{a["pullback"]}%</td></tr>'
    ...
    with b3:
        reasons=''.join(f'<div class="reason"><span>{escape(label)}</span><span class="points {"up" if points>0 else "down"}">{points:+d}</span></div>' for label,points in analysis["breakdown"][:10])
        option_class="option-call" if analysis["signal"]=="CALL WATCH" else "option-put" if analysis["signal"]=="PUT WATCH" else "option-wait"
        option_outlook=f'<div class="option-outlook {option_class}"><b>{escape(analysis["option_bias"])}</b><span>{escape(analysis["option_reason"])}</span></div>'
        st.markdown(f'<div class="panel bottom-panel"><div class="panel-title"><span>Explainable AI · Why This Score?</span><span>Auditable</span></div><div class="panel-body"><div class="reason-grid">{reasons}</div>{option_outlook}<div class="score-total"><span><b>FINAL AI SCORE</b><div class="micro">Base 50 + signed technical contributions</div></span><strong>{analysis["score"]}/100</strong></div></div></div>',unsafe_allow_html=True)
